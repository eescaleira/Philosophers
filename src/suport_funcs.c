...
/*   Updated: 2024/05/16 18:59:12 by eescalei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void handle_mutex_error(int status, t_opcode opcode)
{
	...
	if(EINVAL == status && (LOCK == opcode || UNLOCK == opcode))
		...
		exit_error("A deadlock condition would occur if the thread blocked waiting for mutex");
...
void handle_thread_error(int status, t_opcode opcode)
{
	...
	if(EAGAIN == status)
		...
		exit_error("No thread could be found corresponding to that specified by the given thread ID");
	else if(EDEADLK == status)
